...
def summarize_transcript(transcript):
    ...
    print(len(text))
    if(len(text)>=1000):
        print("looooooooooooong sentence")
        kkma = Kkma()
        sentences = kkma.sentences(text)
        
        sentence = ["".join(sentences[i:i+10]) for i in range(0, len(sentences), 10)]
        len(sentence)
        ...
        for s in sentence:
            ...
            summary_ids = model.generate(torch.tensor([input_ids]),  num_beams=1,  max_length=256,  eos_token_id=1)
        ...
        summary_ids = model.generate(torch.tensor([input_ids]),  num_beams=2,  max_length=1024,  eos_token_id=1)
        result += tokenizer.decode(summary_ids.squeeze().tolist(), skip_special_tokens=True)
